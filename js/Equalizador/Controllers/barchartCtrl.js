...
define([
    ...
    'underscore'
        ...
        app.controller('barchartCtrl',['$scope', '$rootScope', '$q', '$translate', 'orderByFilter', function ($scope, $rootScope, $q, $translate, orderBy) {
            ...
            $scope.app.model.engineApp.getObject(
                ...
                qBook.getEffectiveProperties().then(function(res){
                    ...
                    function getColor() {
                        ...
                        $scope.checkColor = '';
                        $scope.color = _propiedades.color.mode;
                        $scope.modo = _propiedades.color.auto;
                        $scope.dimensions = _propiedades.qHyperCubeDef.qDimensions;
                        $scope.numDimensions = $scope.dimensions.length;

                        if ($scope.color == 'primary' && $scope.modo == true) {
                            $scope.checkColor = 'primary';
                        }
                        if ($scope.color == 'primary' && $scope.modo == false) {
                            $scope.checkColor = 'unico';
                        }
                        if ($scope.color == 'byMultiple' && $scope.modo == false) {
                            $scope.checkColor = 'byMultiple';
                        }
                        if ($scope.color == 'byDimension' && $scope.modo == false) {
                            $scope.checkColor = 'byDimension';
                        }

                        if ($scope.color == 'byMeasure' && $scope.modo == false) {
                    ...
                    const getShowLegendtitle = ()=>{
                        var _propiedades = _model;
                        $scope.checkTitleboxLeyendas = _propiedades.legend.showTitle
                    }
                    $scope.changeLeyendasTitulo = (valor)=>{
                        $scope.checkTitleboxLeyendas = valor;
                        var _patches = [{
                            "qPath": "/legend/showTitle",
                            "qOp": "replace",
                            "qValue": JSON.stringify($scope.checkTitleboxLeyendas)
                        }]
                        $scope.Patches.push(_patches);
                        vis.model.applyPatches(_patches, true);
                    }
