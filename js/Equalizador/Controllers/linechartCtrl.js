...
define([
    ...
    app.controller('linechartCtrl', ['$scope', '$rootScope', '$q', '$translate', 'orderByFilter', function ($scope, $rootScope, $q, $translate, orderBy) {
        ...
        $scope.app.model.engineApp.getObject(
            ...
            qBook.getEffectiveProperties().then(function (res) {
                ...
                $scope.alternativeDimensionItemClicked = async function ($event, dimension, dimensionAlternativa) {
                                ...
                                listOFDimensions.forEach(function (valDimension, key) {
                                    promises.push(
                                        $rootScope.getNombreDimensionAlternativa($scope.ALLDIMENSIONS.filter(obj => { if (obj.qDef.cId == valDimension.qDef.cId) { return obj; } })[0]).then(function (rest) {
                                            ...
                                            ds.sortable = listOFDimensions.length < 2 ? true : false;
                                            ds.name = rest;
                                ...
                                listOFMeasures.forEach(function (valMeasure, key) {
                                    promises.push(
                                        $rootScope.getNombreMeasureAlternativa($scope.ALLMEASURES.filter(obj => { if (obj.qDef.cId == valMeasure.qDef.cId) { return obj; } })[0]).then(function (rest) {
                                            ...
                                            ms.sortable = listOFDimensions.length < 2 ? true : false;
                                            ms.name = rest;
                ...
                function getColor() {
                    ...
                    $scope.isbyMeasure = _propiedades.color.hasOwnProperty('byMeasureDef');
                    ...
                    if ($scope.colorTipo == 'byMeasure' && $scope.modoColor == false) {
